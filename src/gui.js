#!/usr/bin/env node
// QVAC Fake Scientific Study Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29368;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/fakestudy") {
      try {
        ...
        const result = await generate(modelId, habit.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Scientific Study Generator GUI ready at http://localhost:${PORT}`);
  });
